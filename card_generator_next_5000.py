...
def load_existing_words():
    """Load all words from the existing 5000-word set to avoid duplicates."""
    existing_words = set()
    
    # Check if the 5000-word set exists
    if os.path.exists("flashcard_sets/5000_common_words_complete.json"):
        ...
        print("Warning: 5000_common_words_complete.json not found. Starting with empty word set.")
    
    return existing_words

def generate_flashcard_set(set_number, existing_words, max_retries=3, timeout=300):
    """Generate a flashcard set with 100 words, avoiding existing words."""
    # Create context from existing words - include all words to avoid duplicates
    context = f"Here are the words already generated (DO NOT REPEAT THESE): {', '.join(list(existing_words))}"
    
    prompt = f"""Generate a JSON object for a Spanish flashcard set with the following specifications:
    - Set name should be "Spanish Common Words Set {set_number} (Next 5000)"
    - Description should explain this is part of the next 5000 most common Spanish words (words 5001-10000)
    - Include exactly 100 cards
    - Each card should have:
        - word: the Spanish word (must be a common, frequently used word, but NOT in the top 5000)
    ...
    Focus on words that are frequently used but ranked between 5001-10000 in frequency.
    {context}
    
    IMPORTANT: Do NOT include any of the words listed above. These are the top 5000 most common words.
    Generate words that are common but slightly less frequent than the top 5000.
            ...
            for i, card in enumerate(data["cards"]):
                ...
                for field in required_fields:
                    if field not in card:
                        print(f"Warning: Card {i} is missing required field: {field}")
...
def combine_flashcard_sets(sets):
    ...
    combined = {
        "name": "Next 5000 Most Common Spanish Words",
        "description": "A comprehensive set of the next 5000 most common Spanish words (words 5001-10000), including nouns, verbs, adjectives, and other parts of speech",
        "cards": []
    }
...
def main():
    ...
    # Load existing words from the first 5000-word set
    ...
    for set_number in existing_sets:
        filename = f"flashcard_sets/next_5000_set_{set_number}.json"
    ...
    while current_word_count < target_word_count:
        print(f"Generating set {set_number} of {target_word_count // words_per_set} (words {current_word_count + 1}-{current_word_count + words_per_set})")
        set_data = generate_flashcard_set(set_number, existing_words)
        
        if set_data:
            # Update existing words with new words from this set
            for card in set_data["cards"]:
                existing_words.add(card["word"])
                current_word_count += 1
            ...
            if current_word_count % 1000 == 0:
                ...
                thousand_set["name"] = f"Next 5000 Spanish Words Set {current_word_count//1000}"
                thousand_set["description"] = f"Set {current_word_count//1000} of 5 - Words {5000 + current_word_count-999} to {5000 + current_word_count} most common Spanish words"
                thousand_filename = f"flashcard_sets/next_5000_set_{current_word_count//1000}_of_5.json"
    ...
    final_combined_set["name"] = "Next 5000 Most Common Spanish Words"
    final_combined_set["description"] = "A comprehensive set of the next 5000 most common Spanish words (words 5001-10000), including nouns, verbs, adjectives, and other parts of speech"
    with open("flashcard_sets/next_5000_common_words_complete.json", 'w', encoding='utf-8') as f:
        json.dump(final_combined_set, f, ensure_ascii=False, indent=2)
    print("Saved final combined set to flashcard_sets/next_5000_common_words_complete.json")
    
    print(f"Total unique words generated: {len(existing_words) - 5000}")  # Subtract the original 5000
    print(f"Final word count: {current_word_count}")
    print("Note: These are words ranked 5001-10000 in frequency, avoiding all words from the top 5000")
